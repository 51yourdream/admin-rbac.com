...
//        }
    ...
    public function update(array $attributes, $id)
    {
        ...
        //$this->model->find($id)->roles()->detach();  //删除 user 指定角色

//        if(isset($attributes['roles'])) {
//            $this->attachRoles($id, $attributes['roles']); //attach 方法简单地把 roles 附加给一个 user
//        }
    ...
    public function assignRoles(array $attributes,$id)
    {
        $this->model->find($id)->roles()->detach();
        if(isset($attributes['roles'])) {
            if($this->attachRoles($id, $attributes['roles'])){//attach 方法简单地把 roles 附加给一个 user
                return ['status' => false];
            }
        }
        return ['status' => true];
