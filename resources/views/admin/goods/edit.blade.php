...
@section('pageHeading')
    <ul class="breadcrumb">
        {!! Breadcrumbs::render('admin-goods-edit') !!}
    </ul>
@stop
@section('wrapper')
    <div class="row">
                    ...
                    <form role="form" action="{{ route('admin.goods.update',['id'=>$goods->id])}}" enctype="multipart/form-data" method="POST">
                        ...
                        <div class="form-group @if($errors->has('title')) has-error @endif">
                            ...
                            <input required  type="text" data-toggle="tooltip" name="title"
                                   ...
                                   data-original-title="最多255个字符" value="{{$goods->title,''}}">
                        ...
                        <div class="form-group @if($errors->has('price')) has-error @endif">
                            ...
                            <input required type="text" class="form-control" name="price" id="price" onkeyup="this.value=/^\d{0,4}\.?\d{0,2}$/.test(this.value) ? this.value : this.value.substr(0,this.value.length-1)" value="{{$goods->price,''}}">
                        ...
                        <div class="form-group @if($errors->has('stocks')) has-error @endif">
                            ...
                            <input  type="number" class="form-control" name="stocks" id="stocks" value="{{$goods->stocks,''}}">
                        ...
                        <div class="form-group @if($errors->has('pic')) has-error @endif">
                            ...
                            <div class="fileupload fileupload-new" data-provides="fileupload">
                                ...
                                <div class="thumbnail">
                                    <img src="{{asset($goods->pic)}}" alt="{{$goods->title,'没有上传过图片'}}" width="200px" />
                                </div>
                                <div>
                        ...
                        <div class="form-group @if($errors->has('details')) has-error @endif">
                            ...
                            <textarea class="form-control ckeditor" name="details" rows="6">{{$goods->details,''}}</textarea>
                            <p class="help-block">{{$errors->first('details')}}</p>
                        </div>
                        <button type="submit" class="btn btn-primary">{{Lang::get('information.common.edit')}}</button>
